...
so $n\theta=2\pi k+\phi$, where $k\ge0$ is an integer and
$0<\phi<\pi/2$. If $k=0$, every $\alpha^j$, $1\le j\le n$,
...
Put $L=\ln(2n+1)$ and $K_n=2\sinh(L/n)/(L/n)$. Then
\begin{equation}\label{Jbound}
 0<|J|\le K_n a\ln a.
\end{equation}
Moreover, $K_n<21/10$ for $n\ge6$, and $K_n<201/100$ for $n\ge1000$.
\end{lemma}
\begin{proof}
...
\]
Use $0<\ln|\alpha_i|<L/n$, the monotonicity of $\sinh t/t$, and
$\sum_i\ln|\alpha_i|=\ln a$.
The quantity $L/n$ decreases with $n$; it is below $1/2$ for $n\ge6$
and below $1/100$ for $n\ge1000$. The inequality
...
by $p$, and write $M=p^r m$, where $p\nmid m$.
...
\end{lemma}
\begin{proof}
For a monic divisor $A$ of degree $s$, multiplication over its roots
gives $v(A(\zeta))=s/e$. This belongs to the value group $\mathbb Z$
of $E$, so $e\mid s$.
\end{proof}
...
\end{array}
\]
These follow from $\binom{M-1}k\equiv(-1)^k\pmod p$ for $k<p^r$.
...
The roots reducing to $\zeta$ form irreducible packets of degrees
\[
 e_1=p-2\varepsilon,\qquad
 e_j=p^{j-1}(p-1)\quad(2\le j\le r),
\]
where $\varepsilon=1$ if $\zeta=1$, and $\varepsilon=0$ otherwise.
...
a possible quadratic
part of the binary quartic. Its contribution to
...
\[
 R=\ln|\Res(g,h)|,\qquad B=\tfrac12\ln(nN),\qquad
...
\begin{equation}\label{budget-eight}
 R\le\left(\frac N8-2\right)B+NC_8,
 \qquad
 C_8=\tfrac38\ln2+\tfrac5{24}\ln3+
     \tfrac3{40}\ln5+\tfrac1{56}\ln7<\tfrac{13}{20}.
\end{equation}
\end{lemma}
\begin{proof}
...
Put $B_0=\sum_p\lfloor v_p(nN)/2\rfloor\ln p\le B$.
Retain the negative term in \eqref{localbound}, and replace each
$M\in\{n,N\}$ by $N$ in its positive term.
...
\[
 R\le(N/8-2)B_0+NC_8\le(N/8-2)B+NC_8,
\]
where the last step uses $N\ge16$.
The bounds $\ln2<0.7$, $\ln3<1.1$, $\ln5<13/8$ and $\ln7<2$
give $C_8<4363/6720<13/20$.
\end{proof}
...
\[
 \ell=\ln n,\qquad L=\ln(2n+1),\qquad
 A=\ln a,\qquad\delta=d/n.
\]
The annulus gives $0<A/L<\delta<1/2$.
...
\end{lemma}
\begin{proof}
Write $\alpha_i=\rho_i u_i$, with $|u_i|=1$, and put
...
Using $1-e^{-\sigma}\ge\sigma e^{-\sigma/2}$ then gives
$\ln|\Disc(g)|\le(d-1)A+\sigma d^2/2-d\ln\sigma$.
\end{proof}

For such a width define
\[
 G_W=\delta(\ell-A+\ln W)-\tfrac12W\delta^2.
\]
...
\begin{equation}\label{energyineq}
 G_W\le\frac{R+\ln g(1)}n-\delta\ln(N/n).
\end{equation}
\end{lemma}
\begin{proof}
...
$\ln g(1)\le2B-\ln2$.
In \eqref{energyineq}, this cancels the $-2B$ term of
\eqref{budget-eight}. Now $B<\ell+1/(2n)$ and $C_8<13/20$.
The error above $\ell/8+13/20$ is at most
\[
 \frac{\ell/8+13/20+1/16+1/(16n)}n<1/100
 \qquad(n\ge182),
\]
...
Put $K=201/100$ and
\[
 S(A)=\tfrac3{20}A+\tfrac1{10}\ln(KA)+\tfrac{97}{100}.
\]
...
and~\ref{Jsize},
\[
 Q\le\sqrt a\,\rad(bN)\le\sqrt a\,|J|<K a^{3/2}A.
\]
...
$(2.1\ell+1)/n<1/50$ for $n\ge1000$.
\end{proof}

\begin{lemma}[Comparison for large parameters]\label{comparison-large}
If $n\ge1000$ and $51/20\le A<\delta L<L/2$, the two bounds
\eqref{upper1} with $W=L$ and \eqref{upper2} are incompatible.
\end{lemma}
\begin{proof}
Here $\ell>6.9$, $0<L-\ell<0.7$ and $\ln L>2$.
The expression $G_L$ increases with $\delta$ on the stated interval, so
\[
 G_L>H(A):=\frac{A(\ell+\ln L-3A/2)}L.
\]
...
For the first row, $(\ell+2-1.5A)/(\ell+0.7)$ increases with $\ell$,
so
\[
 H(A)>H_0(A):=\frac{A(8.9-1.5A)}{7.6}.
\]
The function $H_0-S$ is concave; its endpoint values exceed $0.18$
and $0.1$, using $\ln(2.55K)<1.7$ and $\ln(3K)<2$.
For the second row, $H(A)>5A/8$, and
$19A/40-\ln(KA)/10-0.97$ increases for $A\ge3$ and exceeds $0.255$
at three.

For the last row use concavity of $H$. At $A=L/2$,
\[
 H(A)-\ell/8=\tfrac12\ln L-\tfrac38(L-\ell)>59/80.
\]
At the other endpoint, for $6.9\le\ell\le12$, a lower bound is
$3(\ell-2.5)/(\ell+0.7)-\ell/8$.
This is concave in $\ell$, with endpoint values $1329/1520$ and
$189/254$, both greater than $59/80$.
For $\ell\ge12$, the lower bound is
...
If $182\le n<1000$, $51/20\le A<\delta L<L/2$, and $W=L-11/6$,
then
\[
 G_W>\ell/8+2/3.
\]
\end{lemma}
\begin{proof}
Put $c=L-\ell$. Here $5.2<\ell<7$, $0.69<c<0.7$ and
$1.4<\ln W<1.8$. Since $G_W$ increases with $\delta$,
\[
 G_W>H_W(A):=\frac{A(\ell+\ln W)}L
             -\frac{A^2(L+W/2)}{L^2}.
\]
...
\]
The polynomial $P$ decreases in $c$, and $P(\ell,0.69)$ increases
in $\ell$ on this rectangle. Its maximum is
\[
 P(7,0.69)=-27869/60000<0.
\]
Consequently
\[
 G_W>H_W(L/2)=\ell/8+\tfrac12\ln W-\tfrac38c+\tfrac{11}{48}
 >\ell/8+\tfrac23.
\]
...
$A\ge\ln13>51/20$ and $A<\delta L<L/2$.
...
roots and $d$ is even.
Multiplying $\alpha^N=N(\alpha-t)$ over the roots of $g$ now gives
$a^N=N^d g(t)$, with the signs cancelling because $nd$ is even.
For $|z|>1$,
\[
 |z-t|^2-t|z-1|^2=(1-t)(|z|^2-t)>0.
\]
Multiplication gives $g(t)>t^{d/2}g(1)\ge t^{d/2}$, and hence
$N^d g(t)>(\sqrt{nN})^d$.
\end{proof}
...
\end{lemma}
\begin{proof}
Suppose $N=p^r$ and choose $g$ of smaller degree. Put $H=\sqrt{nN}$.
...
and~\ref{norm-metric}. This yields $p>\tfrac38H^{1-1/p}$, whereas
$H>p^3-1>\tfrac89p^3$ gives
$H^{1-1/p}>\tfrac89p^2\ge\tfrac83p$, a contradiction.
